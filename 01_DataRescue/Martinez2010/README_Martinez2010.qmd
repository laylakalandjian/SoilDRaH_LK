---
title: "Data Rescue: Martinez 2010"
format:
  html:
    toc: true
...
bibliography:
  - Martinez2010_Methods.bib #contains the citations in the methods section
  - Martinez2010.bib #contains citation for the data source, generally a single citation of the primary paper
authors: #author scheme from https://quarto.org/docs/journals/authors.html
  ...
  - id: laylakalandjian
    ...
    affiliation:
      - ref: uf-ees
    role:
      - transcription: Table 1, Figure 1
  - id: angelacanini-a11y
    name:
      given: Angela
      family: Canini
    affiliation:
      - ref: uf-ees
    role:
      - transcription: Table 1, Table 2, Methods citations
  - id: vaasukimarupaka
    name:
      given: Vaasuki
      family: Marupaka
    affiliation:
      - ref: uf-ees
    role:
      - transcription: Table 1, Methods.md, Methods citations
...
timelog:
  ...
  - activity: 
      ...
      duration: P45M
  - activity: 
      description: UF ESSIE PhD candidate
      who: vaasukimarupaka #github username
      #when you started and ended, can be just a year or down to the minute
      start: 2026-16-06 T1600
      #start: YYYY-MM-DD THH:MM
      #alternatively duration can be combined with a being/end
      duration: P05M
  - activity: 
...
duration: P10M      duration: P10Mversion: 'vs202512' #template version, remove before use
---
...
methods.file <- "Martinez2010_Methods.md"
table1.file <- 'Martinez2010_Table1.csv'
table2.file <- 'Martinez2010_Table2.csv'
figure1.file <- 'Martinez2010_Figure1.csv'

# Bibliography files
primaryCitation.file <- 'Martinez2010.bib'
methodsCitation.file <- 'Martinez2010_Methods.bib'
...
readsource.file <- '../../02_DataHarmonization/readMartinez2010.R'
...
Martínez G, Ingrid, Zagal V, Erick, Ovalle M, Carlos, Coûteaux, Marie-Madeleine, Stolpe, Neal B, & Valderrama V, Natalia. (2010). Litter Decomposition of Acacia caven (Molina) Molina and Lolium multiflorum Lam. in Mediterranean Climate Ecosystems. Chilean journal of agricultural research, 70(3), 454-464. https://dx.doi.org/10.4067/S0718-58392010000300013

For the discussion of this data rescue see the Github issue: https://github.com/ktoddbrown/SoilDRaH/issues/#259.
...
In this study, conducted in the Mediterranean ecosystems of central southern Chile, researchers aimed to evaluate litter decomposition of A. caven and Lolium multiflorum under different cover densities using litter bags stored in 1-m high cages.
...
- Tables
  1) Table 1: Litter chemical composition of Acacia caven and Lolium multiflorum before field incubation.
  2) Table 2: Decomposition rate (k) and weight loss (A) at 420 days for Acacia caven and Lolium multiflorum litter.
  3) Table 3: Correlation matrix of principal components for Acacia caven and Lolium multiflorum.


- Figures
  1) Figure 1: Weight loss as a percentage of initial weight for Acacia caven and Lolium multiflorum in a degraded ecosystem (De), typical ecosystem (Pd), and well preserved ecosystem (Wp) over 450 days.
  2) Figure 2: Two-dimensional graph for PC1 and PC2 of Acacia caven litter with the variables under study
  3) Figure 3: ANOVA (p < 0.001) for PC1 (a) and PC3 (b) factors for Acacia caven.
  4) Figure 4: ANOVA (p < 0.001) PC2 factor for Acacia caven.
  5) Figure 5: Two-dimensional graph for Lolium multiflorum, correlations of PC1 and PC2 principal components with each variable under study.

## Fit for purpose: DeAD

<!--- Write a brief description of the project you are processing this data source for. This could be a copy-paste from other documentation. ---->

The Decomposition Across Drylands (DeAD) data collection is a new data effort targeting dryland decomposition studies. The purpose of this data collection is to develop better litter decay models for dryland ecosystems. This paper has been identified as a source for the information above.
...
- Location: The study was carried out in the Cauquenes Experimental Centre of Instituto de Investigaciones Agropecuarias INIA (35°78’ S; 72°20’ W) located in the interior dryland of the Maule Region.
- Duration: During 420 d and starting in April 2004 until August 2005.
- Mass loss: Weight loss ranged from 31 to 52% in L. multiflorum and 26 to 40% in A. caven after 420 d. Specifics reported in figure 1 and table 2.
- Solar exposure: Three different cover densities: well preserved (Wp) 50 to 80%, typical (Pd) 25 to 50%, and degraded (De) with 10 to 25% cover.
  
## Files

<!--- Include a brief explaination to all the files in the folder--->

These files are in the Martinez2010 data rescue.

- [Readme](README_Martinez2010.qmd)
  + This is the primary file that documents the transcriptions and decision made during data rescue.
- [Martinez2010.bib](Martinez2010.bib)
  + citation for article transcribed
- [Martinez2010_Methods.bib](Martinez2010_Methods.bib)
  + citations for the methods section of the article
- [Martinez2010_Methods.md](Martinez2010_Methods.md)
  + methods transcribed from primary article
- [Martinez2010_Table1.csv](Martinez2010_Table1.csv)
  + table 1 from primary article with litter chemical composition of both litter species before field incubation.
- [Martinez2010_Table2.csv](Martinez2010_Table2.csv)
  + table 2 from primary article with the decomposition rate, weight loss, and regression coefficient of each litter species at each ecosystem.
- [Martinez2010_Figure1.csv](Martinez2010_Figure1.csv)
  + figure 1 from primary article with weight loss as a percentage of initial weight for both species in each ecosystem across 420 days.
- [temp/](temp/)
...
+ Lead
  ...
  - [x] Create a fresh fork of the primary repository https://github.com/ktoddbrown/SoilDRaH/tree/main, it might be useful to rename it 'SoilDRaH_Martinez2010'
  - [x] Create a copy of this template folder and rename it to Martinez2010
...
+ Do
  ...
  - [x] Create a spreadsheet with Figure1 and export it to csv
...
Table 1 was modified by adding Latex and Markdown to denote formatting.
...
diff --strip-trailing-cr --suppress-common-lines -y Martinez2010_Table1.csv temp/Martinez2010_Table1_AC.csv
...
Table 2 was modified by adding Latex and Markdown to denote formatting.
...
# diff --strip-trailing-cr --suppress-common-lines -y Martinez2010_Table2.md Martinez2010_Table2_AC.md

diff --strip-trailing-cr --suppress-common-lines -y Martinez2010_Table2.csv temp/Martinez2010_Table2_AC.csv
...
```{bash}
#| label: compareFigure1Table
#| eval: true

# You will want to repeat the line below to cross check each csv and md file in the
#... data rescue. Having no lines flagged by this command will ensure that each character matches exactly.

diff --strip-trailing-cr --suppress-common-lines -y Martinez2010_Figure1.csv temp/Martinez2010_Figure1_MA.csv
```

Now let's manipulate dataframe for plotting.

```{r}

figure1.df <- read_csv(file = figure1.file, #read in the specific file
         skip = 1, #skip the first line with the caption
         #read everything as a character, don't worry about formatting yet
         col_types = cols(.default = col_character())) |>
  mutate(`Ecosystem Degredation Level` = factor(`Ecosystem Degredation Level`,
                                                levels = c("Wp 2", "Wp 1", "Pd 2", 
                                                           "Pd 1", "De 2", "De 1"))) |>
  mutate(across(c(`X Value`, `Y Value`, 
                  `Upper Bound Y Value`), as.numeric)) |>
  mutate(bound = `Upper Bound Y Value` - `Y Value`,
         lower_bound = `Y Value` - bound) |> 
  mutate(Figure = case_when(str_detect(Figure, 'A') ~ 'Acacia caven',
                            str_detect(Figure, 'B') ~ 'Lolium multiflorum'))

# make the plot
ggplot(data = figure1.df, aes(x = `X Value`, y = `Y Value`,
       shape = `Ecosystem Degredation Level`)) +
  geom_point(size = 0.8) +
  geom_errorbar(aes( ymin = lower_bound, ymax = `Upper Bound Y Value`), width = 5.5) +
  scale_y_continuous(breaks = seq(40, 110, by = 10), limits = c(40, 110)) +
  facet_wrap(~Figure, ncol = 1) +
  scale_x_continuous(breaks = seq(0, 450, by = 50), limits = c(0, 450)) +
  labs(y = 'Mass loss (% initial DW)',
       x = 'Time(days)') +
  theme_bw()
...
# You will want to repeat the line below to cross check each csv and md file in the
#... data rescue. Having no lines flagged by this command will ensure that each character matches exactly.
diff --strip-trailing-cr --suppress-common-lines -y Martinez2010_Methods.md temp/Martinez2010_Methods_VM.md
```
...
if(file.exists(readsource.file)){
  datafunction.lvl0.ls <- readMartinez2010(dataDir = '.',
                                         dataLevel = 'level0')
...
if(file.exists(readsource.file)){
  datafunction.lvl1.ls <- readMartinez2010(dataDir = '.',
                                         dataLevel = 'level1')
